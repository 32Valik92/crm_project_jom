...
function FormContent({schema, block, initialValues, onCancel, onSave, page}: Props) {
	...
	return (
		<form onSubmit={onSubmit} className="w-[92vw] max-w-[900px] overflow-hidden rounded-2xl">
			<div
				className="sticky top-0 z-10 border-b border-slate-200/70 bg-gradient-to-r from-sky-50 to-white dark:from-sky-950/30 dark:to-transparent dark:border-white/10">
				<div className="px-6 py-4">
					<h3 className="text-[18px] sm:text-[20px] font-semibold leading-[28px] tracking-tight">
            <span
	            className="bg-gradient-to-r from-sky-600 to-sky-400 bg-clip-text text-transparent dark:from-sky-300 dark:to-sky-200">
              {block}
            </span>
					</h3>
			...
			<div
				className="sticky bottom-0 z-10 border-t border-slate-200/70 bg-white/90 backdrop-blur px-6 py-4 flex justify-end gap-3 dark:bg-[#0f172a]/90 dark:border-white/10">
				<button
					type="button"
					onClick={onCancel}
