// src/components/validator_json/forms/bonus_promocode/AboutPrimaryForm.tsx
"use client";

import { useEffect, useRef } from "react";
import { type Control, type UseFormRegister, useFieldArray } from "react-hook-form";

export default function BonusPromocodeAboutPrimaryForm({
                                                           control,
                                                           registerAction,
                                                       }: {
    control: Control;
    ...
    useEffect(() => {
        ...
        if (blocksFA.fields.length === 0) blocksFA.append({ title: "", imageHero: "", intro: "", items: [] });
            ...
            <div className="space-y-[8px]">
                ...
                {itemsFA.fields.map((it, iIdx) => (
                    <div key={it.id} className="flex items-center gap-[8px]">
                        <input className="w-full rounded-[4px] border p-[8px]" placeholder="item" {...registerAction(`blocks.${idx}.items.${iIdx}`)} />
